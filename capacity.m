% function [c_sum,t,a,b,c,d,e,f,rate,dict,deco_overhead,size_deco_overhead,permark,loc_num,overhead,location,a_location,cal]=capacity(img,mode_new,h,w)
 function [c_sum,cal,location,overhead,LUT,len_0,len_1]=capacity(img,mode_new,h,w)
...
for k=1:10
    ...
    LUT=[LUT fliplr(dec2binvec(cal(k).list1,16)) fliplr(dec2binvec(cal(k).list2,16))];
end
